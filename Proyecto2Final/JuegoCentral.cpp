...
void JuegoCentral::tipoEstrategia() {
	cout << "---------------Tipo de estrategia---------------" << endl;
	cout << "Juego Central" << endl << endl;
}

int JuegoCentral::cantiColumnas() {   //HACER LO DE SEPARADOR
...
int JuegoCentral::filaAleatoria() {
	int fi;
	int filaAle;
	filaAle = 3;
	srand(time(NULL));
...
string JuegoCentral::matrizJuego() {
	string x = "\nEspacio de juego\n";
	int col = cantiColumnas();
	int fi = 3;
	for (int i = 1; i <= col; i++) {
...
void JuegoCentral::turnoJugador() {
...
void JuegoCentral::estrategiaCentral() {  //recibe una matriz y le coloca una linea en cualquier lugar									  						
	//Juego
