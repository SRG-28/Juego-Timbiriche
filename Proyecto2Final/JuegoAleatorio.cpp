...
void JuegoAleatorio::tipoEstrategia() {
	cout << "---------------Tipo de estrategia---------------" << endl;
	cout << "Juego Aleatorio" << endl << endl;
}

int JuegoAleatorio::cantiColumnas() {   //HACER LO DE SEPARADOR
...
int JuegoAleatorio::filaAleatoria() {
	int fi;
	int filaAle;
	filaAle = 3;
	srand(time(NULL));
...
int JuegoAleatorio::colAleatoria() {
	int co;
	int colAle;
...
string JuegoAleatorio::matrizJuego() {
	string x = "\nEspacio de juego\n";
	int col = cantiColumnas();
	int fi = 3;
	for (int i = 1; i <= col; i++) {
...
void JuegoAleatorio::turnoJugador() {
...
void JuegoAleatorio::estrategiaAleatoria() {  //recibe una matriz y le coloca una linea en cualquier lugar									  						
	//Juego
